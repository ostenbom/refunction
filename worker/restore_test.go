...
var _ = Describe("Worker Restoring", func() {
	...
	Describe("changing the stack", func() {
		var worker *Worker
	...
	Describe("changing registers", func() {
		var worker *Worker
		var targetLayer string
		...
		It("can not restore variable in a register by memory", func() {
			countLocation := getRootfs(worker, targetLayer) + "count.txt"
			...
			Expect(err).NotTo(HaveOccurred())
			numberPrintedIncrements := strings.Count(string(countContent), incrementedLine)
			Expect(numberPrintedIncrements).To(Equal(1))
		})

		It("can restore variable in a register by registers", func() {
			countLocation := getRootfs(worker, targetLayer) + "count.txt"
